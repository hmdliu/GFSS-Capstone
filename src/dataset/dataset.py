...
def get_val_loader(args, episodic=True, return_path=False):
    ...
    if episodic:
        ...
        novel_class_list = filter_classes(args.train_name, args.train_split, test_name, test_split, split_classes)  # cross domain only

        # ====== Build episodic loader ======
        val_data = EpisodicData(mode_train=False, transform=val_transform, novel_class_list=novel_class_list, args=args)
...
class EpisodicData(Dataset):
    def __init__(self,
        ...
        self.base_class_list = list(set(list(range(1, 21 if args.train_name == 'pascal' else 81))) - set(self.novel_class_list))
        if mode_train:    # args.train_list: a txt file that stores all train splits
            self.data_list, self.sub_class_file_list = make_dataset(args.data_root, args.train_list, self.novel_class_list)
            self.base_data_list, self.base_sub_class_file_list = make_dataset(args.data_root, args.train_list, self.base_class_list)
        else:
            self.data_list, self.sub_class_file_list = make_dataset(args.data_root, args.val_list, self.novel_class_list)
            self.base_data_list, self.base_sub_class_file_list = make_dataset(args.data_root, args.val_list, self.base_class_list)
        self.transform = transform
    ...
    def __getitem__(self, index):
        ...
        new_label_class = []
        for c in label_class:
            if c in self.novel_class_list:  # current list of classes to try
                new_label_class.append(c)
        label_class = new_label_class
        assert len(label_class) > 0
        ...
        new_label[target_pix] = target_class_val
        qry_label = new_label
        ...
        support_image_list = []
        support_label_list = []
        novel_class_id = self.novel_class_list.index(class_chosen) + 1
        ...
        # ====== Build base ======
        base_class = np.random.choice(self.base_class_list)
        base_class_id = self.base_class_list.index(base_class) + 1
        base_file_class_chosen = self.base_sub_class_file_list[base_class]
        base_idx = random.randint(1, len(base_file_class_chosen)) - 1
        base_image_path, base_label_path = base_file_class_chosen[base_idx]
        base_img = cv2.imread(base_image_path, cv2.IMREAD_COLOR)
        base_img = np.float32(cv2.cvtColor(base_img, cv2.COLOR_BGR2RGB))
        base_label = cv2.imread(base_label_path, cv2.IMREAD_GRAYSCALE)
        target_pix = np.where(base_label == base_class)
        ignore_pix = np.where(base_label == 255)
        base_label[:, :] = 0
        base_label[target_pix[0], target_pix[1]] = 1
        base_label[ignore_pix[0], ignore_pix[1]] = 255

        # ============== Forward images through transforms
        if self.transform is not None:
            base_img, base_label = self.transform(base_img, base_label)     # transform base img
            qry_img, qry_label = self.transform(image, qry_label)           # transform query img
        ...
        return qry_img, qry_label, spprt_imgs, spprt_labels, base_img, base_label, novel_class_id, base_class_id
