...
def validate_epoch(args, val_idx, backbone, meta_model, val_loader):
    ...
    for episode in range(1, args.test_num+1):
        ...
        try:
            img_q, label_q, img_s, label_s, img_b, label_b, cls_n, cls_b = iter_loader.next()
        except:
            iter_loader = iter(val_loader)
            img_q, label_q, img_s, label_s, img_b, label_b, cls_n, cls_b = iter_loader.next()
        cls_n, cls_b = cls_n.item(), cls_b.item()
        ...
        if torch.cuda.is_available():
            ...
            img_b = img_b.cuda()        # [1, 3, im_size, im_size]
            label_b = label_b.cuda()    # [1, im_size, im_size]
        ...
        for i in range(n_pred):
            ...
            all_intrs[i] += intrs
            all_union[i] += union
            cls_intrs[i][cls] += intrs[1]
            cls_union[i][cls] += union[1]
            mIoU_meter[i][cls] = cls_intrs[i][cls] / (cls_union[i][cls] + 1e-10)
            loss_meter[i].update(loss[i].item())
